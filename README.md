...
- `addSnapshotListener({ window, asset, listener }): Promise<string>`
- `removeSnapshotListener(listenerId): void`
...
### `addSnapshotListener({ window, asset, listener })`

- registers a real-time listener keyed by `window/asset`
- does **not** replay existing snapshots from the current market
- emits only snapshots generated by **new incoming events** after registration
- returns `listenerId` for removal

### `removeSnapshotListener(listenerId)`

- unregisters a listener previously created by `addSnapshotListener`
